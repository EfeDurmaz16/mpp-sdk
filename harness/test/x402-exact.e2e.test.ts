...
describe("x402 exact intent — cross-language matrix", () => {
  ...
  // matrix only self-pairs (ts<->ts, rust<->rust) because the TS
  // reference fixture carries a stub credential. A skipped cross pair
  // must not read as asserted cross-language coverage. Surface the
  // un-asserted cross pairs as a single tracked, logged marker so the
  // gap is visible in the run output instead of looking green.
  ...
  it("cross-language x402 settlement is a tracked, un-asserted gap", () => {
    ...
    // Self-documenting assertion: the matrix only self-pairs today.
