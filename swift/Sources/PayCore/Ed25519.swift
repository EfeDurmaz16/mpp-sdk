...
public enum Ed25519 {
    ...
    public static func privateKey(from raw: Data) throws -> Curve25519.Signing.PrivateKey {
        ...
        default:
            throw PayCoreError.signingFailure(
        ...
        do {
            ...
            throw PayCoreError.signingFailure("CryptoKit rejected secret key: \(error)")
        }
    }
    ...
    public static func sign(message: Data, with privateKey: Curve25519.Signing.PrivateKey) throws -> Data {
        do {
            ...
            throw PayCoreError.signingFailure("CryptoKit signature failed: \(error)")
        }
    }
    ...
    public static func verify(signature: Data, message: Data, publicKey: Data) throws -> Bool {
        guard signature.count == signatureLength else {
            throw PayCoreError.signingFailure("signature must be \(signatureLength) bytes")
        }
        guard publicKey.count == publicKeyLength else {
            throw PayCoreError.signingFailure("public key must be \(publicKeyLength) bytes")
        }
        do {
            let key = try Curve25519.Signing.PublicKey(rawRepresentation: publicKey)
            return key.isValidSignature(signature, for: message)
        } catch {
            throw PayCoreError.signingFailure("CryptoKit rejected public key: \(error)")
        }
    }
