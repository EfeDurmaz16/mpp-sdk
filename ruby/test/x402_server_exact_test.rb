...
class X402ServerExactTest < Minitest::Test
  ...
  def test_v1_producer_emits_legacy_payment_envelope
    state = build_state
    # Bind an explicit memo so the proof is deterministic (without one,
    # build_transaction injects a random memo, which is fine on the wire
    # but makes byte-for-byte proof comparison non-deterministic).
    requirement = X402::Server::Exact.exact_requirement(state, resource: "/protected")

    legacy_header = X402::Protocol::Schemes::Exact.build_exact_payment_signature_legacy(
    ...
    # same memo-bound requirement so the proofs are deterministic.
    v2_header = X402::Protocol::Schemes::Exact.build_exact_payment_signature(
      requirement: requirement,
      client_secret_key: JSON.generate(secret(1)),
      recent_blockhash: BLOCKHASH
    )
  ...
  def test_legacy_network_string_mapping
    map = ->(req) { X402::Protocol::Schemes::Exact.legacy_network_for_requirement(req) }
  ...
  def test_server_accepts_v1_payment_signature
    ...
    assert_equal "v1-settlement",
      X402::Server::Exact.settle_exact_payment(state, legacy_header)
  end
  ...
  def test_server_rejects_v1_with_wrong_scheme
    ...
    error = assert_raises(RuntimeError) do
      X402::Server::Exact.settle_exact_payment(state, tampered)
    end
