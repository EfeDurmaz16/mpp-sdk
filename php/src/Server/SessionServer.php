...
final class SessionServer
{
    ...
    public function createChallenge(SessionRequest $request, string $expires = '', string $digest = '', ?string $opaque = null): Challenge
    {
    ...
    public function createChallengeHeader(SessionRequest $request, string $expires = '', string $digest = '', ?string $opaque = null): string
    {
    ...
    public function verifyAuthorizationHeader(
        ...
        if ($challenge->method !== $this->method || $challenge->intent !== 'session') {
        ...
        try {
            SessionRequest::fromArray($challenge->decodeRequest());
        } catch (InvalidArgumentException $error) {
            return VerificationResult::failure($error->getMessage());
        }
