...
module X402ExactClientFixture
  ...
  # Build a client-signed x402 *legacy v1* payment envelope. The proof is
  # byte-for-byte identical to the v2 producer above (same
  # `build_transaction`); only the envelope differs:
  #   - `x402Version` is 1
  #   - top-level `scheme` is "exact" (present)
  #   - top-level `network` is the legacy network string (section 5 of the
  #     spec; see `legacy_network_for_requirement`)
  ...
  # Map a payment requirement to its legacy v1 network string. Selector is
  # the requirement's `cluster` when present, otherwise `network`. Devnet
  # (in any of its spellings, including the devnet CAIP-2 ID) collapses to
  # "solana-devnet"; every other value (mainnet, testnet, localnet,
  # unknown) collapses to "solana". Mirrors the spine
